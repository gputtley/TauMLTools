...
std::shared_ptr<TauJetSelector> TauJetSelector::Make(const std::string& name)
{
    ...
    if(name == "VSmuAdversarial")
        return std::make_shared<VSmuAdversarial>();
...
TauJetSelector::Result VSmuAdversarial::Select(const edm::Event& event, const std::deque<TauJet>& tauJets,
                                               const std::vector<pat::Electron>& electrons,
                                               const std::vector<pat::Muon>& muons, const pat::MET& met,
                                               const reco::Vertex& primaryVertex,
                                               const pat::TriggerObjectStandAloneCollection& triggerObjects,
                                               const edm::TriggerResults& triggerResults, float rho)
{
    ...
    for(const TauJet& tauJet : tauJets) {
        ...
        for(const pat::Muon& muon : muons) {
            if(!(reco::deltaR(muon.polarP4(), tau.polarP4()) < 0.3 && (tau.charge() == muon.charge()) && ((muon.polarP4()+ref_muon->polarP4()).M() < 110) 
                && ((muon.polarP4()+ref_muon->polarP4()).M() > 70))) continue;
            if(!selectedTau || selectedTau->tau->tauID("byDeepTau2017v2p1VSjetraw")< tau.tauID("byDeepTau2017v2p1VSjetraw") // may want to change this to make it prefer muon fakes
                || (selectedTau->tau->tauID("byDeepTau2017v2p1VSjetraw")== tau.tauID("byDeepTau2017v2p1VSjetraw") && selectedTau->tau->pt() < tau.pt())){
                    selectedTau = &tauJet;
            }
        }
